...
display_help() {
    ...
    echo -e "${yellow}\tExample: $0 start app1${nc}"
    echo
    ...
    echo -e "${yellow}\tExample: $0 stop app1${nc}"
    echo
    ...
    echo -e "${yellow}\tExample: $0 reload app1${nc}"
    echo
    ...
    echo -e "${yellow}\tExample: $0 restart app1${nc}"
    echo
    ...
    echo -e "${yellow}\tExample: $0 undeploy${nc}"
    echo
...
print_status() {
  for item in $app_status;
    do
      ...
      if [ "$status" == "running" ];
        then
          echo -e "${green}$(log_info) $name is $status${nc}"
      elif [ "$status" == "stopped" ];
        then
          echo -e "${red}$(log_info) $name is $status${nc}"
      fi
  done
}
